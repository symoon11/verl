...
class DataParallelPPOActor(BasePPOActor):
    ...
    def _get_last_hidden_state(self, micro_batch):
        response_length = micro_batch["responses"].size(-1)
        multi_modal_inputs = {}
        if "multi_modal_inputs" in micro_batch.keys():
            if "image_bound" in micro_batch["multi_modal_inputs"][0]:  # minicpm-o logic
        ...
        with torch.autocast(device_type=self.device_name, dtype=torch.bfloat16):
            ...
            if self.use_remove_padding:
                ...
                output = self.actor_module._fsdp_wrapped_module.model(
                    input_ids=input_ids_rmpad,
                    attention_mask=None,
                    position_ids=position_ids_rmpad,
                    **multi_modal_inputs,
        ...
        for _ in range(self.config.ppo_epochs):
            for batch_idx, mini_batch in enumerate(mini_batches):
                ...
                for micro_batch in micro_batches:
                    ...
                    policy_loss, aux_outputs = _FSDPForwardRedirection()(
                        self.actor_module,
                        policy_loss_fn,
                        last_hidden_state,
                    ...
                    micro_batch_metrics.update(
                        {
                            "actor/policy_loss": policy_loss.detach().item() * loss_scale_factor,
